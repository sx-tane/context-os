...
const (
	...
	metadataUploadID                = "filesystem_upload_id"
	metadataUploadRoot              = "filesystem_upload_root"
	metadataUploadFiles             = "filesystem_upload_file_count"
	metadataUploadName              = "filesystem_upload_original_name"
)
...
// Upload handles POST /filesystem/upload by staging browser-uploaded files or folders
...
func Upload(w http.ResponseWriter, r *http.Request) {
	...
	metadata := map[string]string{
		metadataUploadID:    staged.UploadID,
		metadataUploadRoot:  staged.Root,
		metadataUploadFiles: strconv.Itoa(staged.FileCount),
	}
	if staged.OriginalName != "" {
		metadata[metadataUploadName] = staged.OriginalName
	}
...
func stageUpload(files []uploadFileEntry) (stagedUpload, error) {
	uploadID, err := newUploadID()
	if err != nil {
		return stagedUpload{}, err
	}

	root := filepath.Join(uploadRoot(), uploadID)
	relativePaths := make([]string, 0, len(files))
	seen := map[string]struct{}{}
	for _, file := range files {
		relativePath, err := cleanRelativePath(file.RelativePath)
		if err != nil {
			_ = os.RemoveAll(root)
			return stagedUpload{}, err
		}
		if _, exists := seen[relativePath]; exists {
			_ = os.RemoveAll(root)
			return stagedUpload{}, fmt.Errorf("%w: duplicate path %q", errInvalidUploadPath, relativePath)
		}
