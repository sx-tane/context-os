...
func sourceDiscoveryPrompt(connector string) string {
	...
	case "jira":
		return `Use the Atlassian Rovo Codex plugin to list Jira projects available to the connected Atlassian account. Use Atlassian Rovo's Jira JQL issue search tool first, not generic Rovo workspace search; generic Rovo search can be blocked even when Jira JQL works. Derive project keys from accessible Jira issues using a read-only JQL query such as ORDER BY updated DESC, and do not modify Atlassian or Jira. Return only compact JSON in this exact shape: {"sources":[{"id":"ABC","label":"ABC — Project name","uri":"https://example.atlassian.net/jira/software/c/projects/ABC","kind":"project"}]}. Prefer project URLs for uri when available; otherwise use the project key. Include at most 100 projects.`
	case "slack":
