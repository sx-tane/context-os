...
The handler uses `http.MaxBytesReader` for bounded JSON input and delegates intent classification, repository reads, source URI resolution, progress callbacks, and Codex live lookup decisions to `internal/chat.Service`. Live chat queries do not have a fixed API deadline; they run until the client request is canceled or Codex returns, so long plugin lookups do not falsely fail with a handler timeout.
