...
// CodexStatus handles GET /codex/status.
...
func CodexStatus(w http.ResponseWriter, r *http.Request) {
...
// CodexLogin handles POST /codex/login.
...
func CodexLogin(w http.ResponseWriter, r *http.Request) {
...
func CodexPluginReauth(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		response.WriteError(w, http.StatusMethodNotAllowed, "method_not_allowed", "POST required")
		return
	}
...
func runCodexSSE(ctx context.Context, sw *sseWriter, f http.Flusher, binary string, args ...string) error {
	cmd := exec.Command(binary, args...) //nolint:gosec
	cmd.SysProcAttr = &syscall.SysProcAttr{Setpgid: true}
	cmd.Stdout = sw
	cmd.Stderr = sw
