...
- `presentation.go`: presentation findings response payload, including event, entity, relationship, actionable mismatch, review candidate, severity, role-view, PMO, and execution evidence counts for frontend findings summaries. `mismatches` is reserved for top actionable issues; generic dependency review rows are returned in `review_candidates`.
- `workspace.go`: `Workspace`, `WorkspaceList`, `WorkspaceStatus`, and `WorkspaceSync` response projections, including registered workspace metadata, local event/entity/relationship/mismatch/audit counts, connector sync rows, source URIs, cursors, statuses, and last errors.
...
- Keep workspace response structs as API projections over repository records instead of returning store structs directly.
