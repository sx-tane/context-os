...
apps/api/
  ...
  docs/
    ...
    swagger.json   — generated OpenAPI 2.0 spec; gitignored
...
5. Regenerate docs (required before building): `swag init -g apps/api/main.go -o apps/api/docs && npx @redocly/cli build-docs apps/api/docs/swagger.yaml --output apps/api/docs/api.html --title "ContextOS API"`. Done automatically by `start-all.sh`.
...
Or use `start-all.sh` which runs `swag init` automatically before starting.
