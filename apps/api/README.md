...
5. Regenerate docs (required before building): `swag init -g apps/api/main.go -o apps/api/docs`. Then refresh frontend TypeScript types: `cd apps/frontend && bun run codegen`. All steps run automatically via `start-all.sh`.
...
Or use `start-all.sh` which runs `swag init` and `bun run codegen` automatically before starting.
