...
apps/api/
  ...
  _docs/
    docs.go        — optional generated docs from swag init; gitignored
    swagger.json   — optional generated OpenAPI 2.0 spec; gitignored
    swagger.yaml   — optional generated YAML spec; gitignored
    api.html       — optional standalone Redoc HTML; gitignored
```
...
3. Add the HTTP handler in `handler/`; include full swag annotations (`@Summary`, `@Tags`, `@Accept`, `@Produce`, `@Param`, `@Success`, `@Failure`, `@Router`) when maintaining generated API docs.
4. Register the route in `main.go` — the `@Router` tag must exactly match.
5. Optionally regenerate standalone docs: `swag init -g apps/api/main.go -o apps/api/_docs && npx @redocly/cli build-docs apps/api/_docs/swagger.yaml --output apps/api/_docs/api.html --title "ContextOS API"`. Generated docs are ignored and are not required for `go run ./apps/api`.
...
Generated OpenAPI/Redoc artifacts under `apps/api/_docs/` are optional local files. Missing generated docs must not block API startup. The underscore keeps generated Go docs out of normal `go test ./...` and `go mod tidy` runs.
