...
Chat replies are answer-first: the direct answer appears before any source summary, and stream or trace details stay collapsed in the transcript. Stopping a live chat run leaves one clean stopped row and suppresses late fallback failures from that run. The composer grows with long prompts up to the available chat space before scrolling internally. Analysis completion replies show counts plus the top three actionable issues; the full decision queue lives in the Findings tab.
