...
- Treat browser storage as unavailable during SSR or malformed Node storage shims so the route can render from default in-memory state.
...
| `storage.ts` | Shape-guarded browser storage access shared by workspace persistence so SSR and Node storage shims fall back cleanly. |
...
- Storage access must stay shape-guarded. SSR and test processes can expose non-browser `localStorage` globals, and the store should fall back to default workspace state instead of calling partial shims.
