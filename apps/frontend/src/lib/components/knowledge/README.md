...
- The panel requires DB-backed workspace routes before saving sources. If `/workspace/status` is unavailable, show a Local DB warning and disable connector registration, filesystem upload, and reset controls instead of surfacing raw 404 errors from `/workspace/source`.
- External connector rows are directly clickable. Enabling a live connector without a URI saves a chat-only connector scope after Codex account/plugin readiness has been checked by the route. Typing a repo, project, issue, channel, page, folder, site, or document URI saves that concrete source so findings analysis can run against it.
- External sources use connected-source language and show `connected` rather than ingest event counts until explicit analysis or ingest creates local artifacts. Connector-only live scopes show `chat only` because findings analysis requires concrete source scope.
- Filesystem remains the setup path that ingests local content into ContextOS storage immediately. Its row exposes browser file and folder upload controls first, with server-path ingest collapsed under a fallback details panel. Browser-uploaded filesystem sources are shown in the saved-source list and must not be prefilled into the footer's live connector save flow on later visits.
