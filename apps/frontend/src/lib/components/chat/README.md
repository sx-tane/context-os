...
The composer stays one line tall so it does not resize the chat footer. Enter and the send button submit, and pasted line breaks are collapsed into spaces. `ChatPanel.svelte` accepts synchronous or async clear handlers so the route can clear browser chat and best-effort backend chat session state from one control.
