...
Pending query progress lives in `ChatMessage.stream` instead of the answer text. `ChatStreamPanel.svelte` renders it as a collapsed stream block by default: running and completed streams show status, the latest Codex-style `>` or `*` progress line, and the final summary when available. The Show/Hide stream toggle must keep pointer, focus, hover, and active affordances visible so it feels clickable during long streams. Expanding the block shows the request, selected mode, inferred route, bounded stream transcript, and final save summary without re-rendering the heavier source evidence UI.

The composer starts as a compact one-line textarea aligned to the send/stop button, then the textarea grows with pasted prompts until it reaches the available chat space and scrolls internally. A compact mode switch above the textarea exposes `Auto`, `Codex`, and `Local`: Auto is the default Codex-then-Local route, Codex is live-only, and Local is persisted-evidence-only. Enter and the send button submit, while Shift+Enter inserts a new line for longer source questions or pasted snippets. `ChatPanel.svelte` accepts synchronous or async clear handlers so the route can clear browser chat and best-effort backend chat session state from one control.
