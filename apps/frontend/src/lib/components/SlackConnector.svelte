<script lang="ts">
  ...
  import { getJSON, postIngest, streamCodexIngest, streamCodexReauth } from "$lib/api";
  ...
  async function runIngest() {
    ...
    if (provider === "codex") {
      ...
      try {
        await streamCodexIngest("slack", { uri, token: token || undefined, provider: "codex" }, {
          onLog: (line) => {
            liveLog += line + "\n";
          },
          onStatus: (seconds) => {
    ...
    try {
      const res = await postIngest("slack", { uri, token: token || undefined, provider });
      if (!res.ok) {
