<script lang="ts">
  ...
  let uri = "inline.txt";
  let token = "";
  let content = "frontend expects refundStatus but backend exposes missingRefundState";
...
<main>
  <header>
    ...
    <p>
      Graph-backed role summaries for delivery intelligence. <a href="/">Open Debug Connectors</a>
    </p>
  </header>
  ...
  <section class="card">
    ...
    <label>
      Content
      <textarea bind:value={content} rows="5" placeholder="Optional inline content for local analysis" />
    </label>
  ...
  {#if result}
    <section class="card">
      ...
      <p><strong>Severity:</strong> high {result.severity_count.high}, medium {result.severity_count.medium}, low {result.severity_count.low}</p>
    ...
    <section class="card">
      ...
      <p><strong>Impacts:</strong> {result.pmo.impacts.join(", ") || "none"}</p>
    ...
    <section class="card">
      ...
      <p><strong>Enabled:</strong> {result.execution.enabled ? "yes" : "no"}</p>
      <p><strong>Assistive:</strong> {result.execution.assistive ? "yes" : "no"}</p>
      ...
      {:else}
        <ul>
          {#each result.mismatches as mismatch}
            <li>
              <strong>[{mismatch.severity}] {mismatch.summary}</strong>
              <div>id: {mismatch.id}</div>
              <div>confidence: {mismatch.confidence}</div>
              <div>impact: {mismatch.impact}</div>
              <div>evidence: {mismatch.evidence.join(", ") || "none"}</div>
              <div>recommended: {mismatch.recommended || "none"}</div>
            </li>
          {/each}
        </ul>
      {/if}
