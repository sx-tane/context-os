...
import (
	...
	"context-os/domain/contracts"                    // SourceRequest used to drive the pipeline
	"context-os/internal/ingestion"                  // NewPipeline wraps connectors for the run
	"context-os/internal/pipeline"                   // Run orchestrates all stages
	githubsource "context-os/internal/source/github" // GitHub connector used as the test source
)
...
func TestRunDetectsPotentialMismatch(t *testing.T) {
	...
	if len(result.Entities) == 0 {
		t.Fatal("expected extracted entities") // the content should have produced at least one entity
	}
