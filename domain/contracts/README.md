...
`SourceRequest` is the universal source input envelope. It lets the ingestion pipeline talk to every source connector with the same contract, whether the backing source is GitHub, Slack, Jira, or local filesystem files and folders such as spreadsheets and OpenAPI specs.
...
| `Metadata` | Extra source-specific context that should travel with the event without changing the domain contract.                                                                                                                        | `source_id`, `trace_id`, `object_type`, `object_id`                                                 |
...
`MCPSourceConnector` converts source input into `document.ingested` domain events. Implementations may return multiple events for collection inputs such as folders, and should be idempotent when the same `SourceRequest` is replayed.
