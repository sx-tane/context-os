...
import (
	...
	"context-os/domain/events"   // raw source events produced by ingestion
	"context-os/domain/types"    // Mismatch detected by the reasoning stage
)

// Result is the output produced by a full pipeline run.
type Result struct {
	EventCount    int                        `json:"event_count"`   // raw source events produced by ingestion
	Events        []events.Event             `json:"events"`        // raw source events produced by ingestion
