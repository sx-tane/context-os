...
| #12 Issue 9: Build filesystem MCP connector            | Filesystem connector ingests server-visible files, recursive folders, and browser uploads with content hashes, paths, modified times, skip counts, and replay-safe per-file events. Extracts text from .docx, .pdf, .pptx, .xlsx, .csv, and OpenAPI JSON/YAML specs. |
...
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| Source         | GitHub, Slack, Jira, and filesystem adapters emit provenance-rich ingestion events; filesystem covers browser uploads, recursive folders, spreadsheets, and OpenAPI specs. | Broader replay validation, durable raw snapshots, and additional cloud/source adapters.                |
...
| Reasoning      | Keyword mismatch detection with deterministic finding type, confidence, impact, and source evidence references.                                                            | Richer cross-layer drift rules, affected roles, false-positive evaluation, and recommendation quality. |
| Execution      | Local stub executor with hidden assistive integration in `/presentation/findings`; execution metadata and trace IDs are returned without mutating deterministic findings. | Replace stub with real local executor backend while preserving auditability and assistive-only boundary. |
| Presentation   | Role-specific API/UI output contracts now available via `/presentation/findings` and frontend `/findings`, including PMO view model and evidence visibility.            | Expand narrative quality checks and broader regression fixtures across additional real-world scenarios. |
