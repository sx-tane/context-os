...
- Starts the SvelteKit frontend dev server (`bun run dev`) in the background
...
Note: API process has already exited (scaffold stub). Frontend is still running.
```

The `API process has already exited` note is expected while the API is a scaffold stub that prints one log line and exits.
The frontend keeps running until you press Ctrl+C.
The `SIGTERM` message on frontend exit is normal — it means the process was stopped cleanly.
