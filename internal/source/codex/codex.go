...
func promptFor(plugin, uri string) string {
	...
	case PluginAtlassianRovo:
		return "Use the Atlassian Rovo Codex plugin to read the Jira context identified by " + uri + ". Use Atlassian Rovo's Jira JQL issue search tool first, not generic Rovo workspace search; generic Rovo search can be blocked even when Jira JQL works. Build a focused JQL query from the URI, issue key, project key, or user request, and retry with Jira JQL if generic Rovo search says the app is not installed on this instance. Return the relevant issue or project content with issue keys, summaries, statuses, descriptions, comments, links, fields, changelog entries, timestamps, authors, and source URLs when available. Do not modify Jira or Atlassian data." + labelRules
