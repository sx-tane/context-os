...
// TestCodexAnswererStartsAndResumesWorkspaceSession verifies live chat stores the first Codex session and resumes it on the second turn.
func TestCodexAnswererStartsAndResumesWorkspaceSession(t *testing.T) {
	...
	sessionID, err := answerer.sessions.Load("workspace-1")
	if err != nil {
...
func TestCodexAnswererSeparatesWorkspaceSessions(t *testing.T) {
	...
	first, err := answerer.sessions.Load("workspace-a")
	if err != nil {
		t.Fatalf("Load(workspace-a) error = %v", err)
	}
	second, err := answerer.sessions.Load("workspace-b")
	if err != nil {
...
// TestCodexAnswererRefreshesUnavailableResumeSession verifies a missing stored Codex session starts a fresh session.
func TestCodexAnswererRefreshesUnavailableResumeSession(t *testing.T) {
	...
	if err := answerer.sessions.Save("workspace-1", "old-session"); err != nil {
...
// TestCodexAnswererSerializesSameWorkspaceLiveCalls verifies concurrent turns for one workspace do not race the stored session.
func TestCodexAnswererSerializesSameWorkspaceLiveCalls(t *testing.T) {
	command, logPath, counterPath := fakeCodexCommand(t)
	answerer := CodexAnswerer{command: command, sessions: NewCodexSessionStore(t.TempDir())}
	t.Setenv("FAKE_CODEX_LOG", logPath)
	t.Setenv("FAKE_CODEX_COUNTER", counterPath)
